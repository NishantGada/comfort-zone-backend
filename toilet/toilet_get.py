...
def get_toilet_details():
    ...
    if not toilet_details:
        return (
            jsonify(
                {"success": False, "message": "Toilet does not exist, check toilet_id"}
            ),
            404,
        )
    ...
    cursor.execute(
        "SELECT feature_name FROM toilet_features WHERE toilet_id = %s", (toilet_id,)
    )
...
def get_all_toilets():
    try:
        ...
        for toilet in toilets:
            if isinstance(toilet["open_time"], (datetime.time, timedelta)):
                toilet["open_time"] = str(toilet["open_time"])
            if isinstance(toilet["close_time"], (datetime.time, timedelta)):
                toilet["close_time"] = str(toilet["close_time"])

        return jsonify(
            {
                "success": True,
                "message": "All toilets fetched successfully",
                "data": {"toilets": toilets, "count": len(toilets)},
            }
        )
